...
function wpcf7_flamingo_before_send_mail( $contactform ) {
	...
	$email = isset( $posted_data['cuf_email'] ) ? trim( $posted_data['cuf_email'] ) : '';

	$title = isset( $posted_data['cuf_title'] ) ? trim( $posted_data['cuf_title'] ) : '';
	$firstName = isset( $posted_data['cuf_senderfirst'] ) ? trim( $posted_data['cuf_senderfirst'] ) : '';
	$lastName = isset( $posted_data['cuf_senderlast'] ) ? trim( $posted_data['cuf_senderlast'] ) : '';

	$name =  $title . ' ' . $firstName . ' ' . $lastName;
	$subject = isset( $posted_data['cuf_subject'] ) ? trim( $posted_data['cuf_subject'] ) : '';
